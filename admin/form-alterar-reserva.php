...
<head>
    ...
    <title>Game Rent - Alterar Reserva id = <?= $id ?></title>
    ...
    <main>
        <article class="introducao">
            <ul class="breadcrumb">
                ...
                <li>Alterar Reservas id = <?= $id ?></li>
            </ul>
        </article>
        <h1 class="titulo">Alterar Reserva</h1>
        <section class="tela-cadastro-cliente">
            <form action="alterar-reserva.php" id="form-cadastro" enctype="multipart/form-data" method="POST">
                <div>
                    <label for="devolvido">Estatus da Reserva: </label>
                    <select name="devolvido" id="devolvido" required>
                        <option value="1" <?= $resultado['FINALIZAR_RESERVA'] == 1 ? 'selected' : '' ?>>Em Vigor</option>
                        <option value="0" <?= $resultado['FINALIZAR_RESERVA'] == 0 ? 'selected' : '' ?>>Expirado</option>
                    </select>
                </div>
                <div>
                    <input type="hidden" name="id" value="<?= $resultado['ID_RESERVA'] ?>">
