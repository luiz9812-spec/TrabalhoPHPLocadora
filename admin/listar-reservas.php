<?php 
    ...
    INNER JOIN CLIENTES C ON C.CPF = R.CPF;"; // String com o comando SQL SELECT
...
<body>
    ...
    <main>
        ...
        <table border="1">
            <tr>
                <th>AÇÕES</th>
                <th>ID</th><th>JOGO</th><th>CPF</th><th>CLIENTE</th><th>DATA DA RESERVA</th><th>EXPIRA EM</th><th>STATUS DE RESERVA</th>
            </tr>
            <?php foreach($resultado as $emprestimo) { ?>
            <tr>
                <td>
                    <a href="form-alterar-reserva.php?id=<?= $emprestimo["ID"] ?>">Alterar</a>
                </td>
                <td><?= $emprestimo["ID"] ?></td>
                <td><?= $emprestimo["JOGO"] ?></td>
                <td><?= $emprestimo["CPF"] ?></td>
                <td><?= $emprestimo["NOME"] ?></td>
                <td><?= $emprestimo["DATA_RESERVA"] ?></td>
                <td><?= $emprestimo["PRAZO_RESERVA"] ?></td>
